 ...
 * 
 * On the contolling tablet the sequence is: <br>
 * PopUp "new-board" -> Yes -> SelectNewBoard() -> SendNewBoardNotice() -> StartNewBoard() <br>
 * 
 * On the receiving Tablets:
 * PopUp -> startNewBoard() 
 */
...
function selectNewBoard() {
    ...
    var bIx = boards.length; // new board index
 ...
 * Called on each tablet, either after sending or receiving notice
...
function startNewBoard(bNbr) {
    ...
    boardIx = bIx;              //global current board index
    dealerIx = boardIx % 4;     //global dealerIx 
    vulIx = (Math.floor(boardIx / 4) + dealerIx) % 4; //global vulIx
    ...
    boards[boardIx] = new Board(bNbr, boardIx, sectionId, tId);
}
